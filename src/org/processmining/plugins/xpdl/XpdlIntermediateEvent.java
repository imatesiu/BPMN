...
public class XpdlIntermediateEvent extends XpdlElement {
	...
	public void convertToBpmn(BPMNDiagram bpmn, String id, String name, DirectedGraphNode parent,
		...
		if (parent == null) {
			id2node.put(id, bpmn.addEvent(name, EventType.INTERMEDIATE, eventTriggerMap.get(trigger), EventUse.CATCH, boundaryActivity));
		} else {
			if (parent instanceof Swimlane) {
				id2node.put(id, bpmn.addEvent(name, EventType.INTERMEDIATE, eventTriggerMap.get(trigger), EventUse.CATCH,
