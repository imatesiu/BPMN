...
public class XpdlActivity extends XpdlIdName {
	...
	public void convertToBpmn(BPMNDiagram bpmn, DirectedGraphNode parent, XpdlActivitySets activitySets,
		...
		if (parent == null) {//then either there is no parent or there is a swimlane
			...
			if (parent != null) {//check if there is a parent  swimlane
				if (event != null) {
					...
					if (blockActivity != null) {
						...
						id2node.put(id, bpmn.addActivity(name, (loop != null) && loop.hasType("Standard"), false,
								false, (loop != null) && loop.hasType("MultiInstance"), false, (Swimlane) parent));
					}
				}
			} else {//there is no parent swimlane
				if (event != null) {
					event.convertToBpmn(bpmn, id, name, parent, id2node);
				} else if (route != null) {
					route.convertToBpmn(bpmn, id, name, parent, id2node);
					...
					if (blockActivity != null) {
						...
						id2node.put(id, bpmn.addActivity(name, (loop != null) && loop.hasType("Standard"), false,
								false, (loop != null) && loop.hasType("MultiInstance"), false));
					}
				}
			}
		} else {
			//then it has to be subprocess
			if (event != null) {
				event.convertToBpmn(bpmn, id, name, parent, id2node);
			} else if (route != null) {
				route.convertToBpmn(bpmn, id, name, parent, id2node);
